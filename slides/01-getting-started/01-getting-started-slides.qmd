...
Go to RStudio and run the following: 
:::: {.columns}
...
- **Slightly harder way**: A more common way to install a package is by typing `install.packages("ggplot2")` in the Console pane of RStudio and hitting enter. Note you must include the quotation marks.
...
We will begin by exploring the `atlas` data frame we just loaded to get an idea of its structure. 

Let's unpack this output:

* `A data.frane: 336,776 x 19`: A `tibble` is a kind of data frame used in R. This particular data frame has
    + `336,776` rows
