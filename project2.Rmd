...
- **Due**: At midnight on Friday, September 30, 2022
...
2.	[Angrist, Joshua D., and Victor Lavy. 1999. “Using Maimonides’ Rule to Estimate the Effect of Class Size on Scholastic Achievement,” Quarterly Journal of Economics 114(2): 533–575.](https://economics.mit.edu/files/8273)
...
You will work on RStudio Cloud for this project. Write the replication within a Quarto/RMarkdown file in the **project2** tab in RStudio Cloud.
...
We will construct a graphical regression discontinuity analysis, focusing on the 40 student school enrollment threshold.  See Table 2a and 2b for more guidance.  
...
4.	Regression analysis.  Run the regressions that correspond to your three graphs in 4a and 4b to quantify the discontinuities that you see in the data.  In estimating these regressions, use all the observations with school enrollment less than 80.  Report a 95% confidence interval for each of these estimates.  See Table 2a and 2b for more guidance.  

5.	Recall that any quasi experiment requires an identification assumption to make it as good as an experiment.  What is the identification assumption for regression discontinuity design?  Explain whether your graphs in 4c and 4d are consistent with that assumption.  
 
6.	(To answer this question, read [Angrist and Lavy (1999)](https://economics.mit.edu/files/8273)). If all schools followed the class size rule exactly as described in Angrist and Lavy (1999), how much would you expect class size to change at the 40 student enrollment threshold? Explain why the actual change in class size that you see in the data is less than this.  
...
9.	Compare your estimates in 7 with the estimates from (i) the Tennessee STAR experiment (Chetty et al. 2011) and (ii) data from Sweden (Fredriksson et al. 2013) discussed in lecture. Give two reasons that your estimates might differ from those of these other studies.
...
narrow_data_school <- narrow_data%>%
  group_by(schlcode)%>%
  summarise(by_school, school_enrollment = mean(school_enrollment, na.rm = TRUE))
```
